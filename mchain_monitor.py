...
class Mchain_Monitor():
    def __init__(self):
        ...
        try:
            ...
            self.error_log = open(
                self.__config['global']['error_filename'], 'a')
            if (self.__config['global']['stat_dest'] == 'file'):
                self.logger = LoggerToFile(self.__config['global']['filename'])
        except:
            self.error_log.write(''.join(str(datetime.datetime.now(
            )), ' Error while init! Check config.ini and stat_dest option'))
            sys.exit()
        try:
            self.w3 = Web3(Web3.WebsocketProvider(
                'ws://' + self.__config['global']['api_address'] + ':' + self.__config['global']['api_port']))
            self.w3.middleware_stack.inject(geth_poa_middleware, layer=0)
        except:
            self.error_log.write(''.join(str(datetime.datetime.now()), ' cannot connect to mchain node via ',
                                         self.__config['global']['api_address'], ':', self.__config['global']['api_port']))
...
#        return event_data.hex()
    ...
    def log_loop(self, event_filter, poll_interval):
        while True:
            for event in event_filter.get_new_entries():
                ...
                self.record['system'] = json.loads(self.sys_stat.get_stats())
    ...
    def run(self):
        self.logger.log_event(
            str(datetime.datetime.now()) + ' starting mchain monitoring')
